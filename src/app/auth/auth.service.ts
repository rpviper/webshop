...
export class AuthService {
  private tokenExpirationTimer: any;    // ctrl + click läheb kus on
  private route = 'https://identitytoolkit.googleapis.com/v1/accounts:';
  private key = 'AIzaSyCP76DD7sqIB4kfHeGBmqVFy-UsayYOc2g';    // selle sain firebasist
  loggedInChanged = new BehaviorSubject<boolean>(this.checkIfSignedIn());

  constructor(private http: HttpClient, private router: Router) { }

  private checkIfSignedIn() {
    if (sessionStorage.getItem("authData")) {   // ei ole väga turvaline
      return true;
      } else {
        return false;
      }
